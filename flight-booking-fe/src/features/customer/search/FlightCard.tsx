...
export const FlightCard = ({ flight, onSelect }: FlightCardProps) => {
  ...
  const handleSelectClass = (selectedClass: FlightClass) => {
    if (onSelect) {
        // Nhường quyền quyết định chuyển trang hay lật trang cho SearchPage
      ...
      <div className="p-5 flex flex-col lg:flex-row items-center justify-between gap-6">
        ...
        <div className="flex items-center justify-center gap-6 w-full lg:w-2/4">
          ...
          <div className="flex flex-col items-center px-2 w-40">
            <span className="text-[10px] font-medium text-slate-400 mb-1 uppercase tracking-wider">
              {flight.durationMinutes ? `${Math.floor(flight.durationMinutes / 60)}h ${flight.durationMinutes % 60}m` : flight.duration || 'Bay thẳng'}
            </span>
      ...
      {isExpanded && (
        <div className="bg-slate-50 border-t border-slate-100 p-5 animate-in slide-in-from-top duration-300">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 text-xs text-slate-600">
             <div className="flex items-center gap-2"><Clock size={14}/> Đúng giờ</div>
             ...
             <div className="flex items-center gap-2"><Armchair size={14}/> Ghế êm ái</div>
          </div>
